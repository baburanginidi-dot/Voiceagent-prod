        ...
        <div className="flex-1 overflow-y-auto space-y-4 pr-2">
            {transcripts.map((message, index) => (
                <div key={index} className={`flex flex-col ${message.speaker === 'User' ? 'items-end' : 'items-start'}`}>
                    <div className={`rounded-xl p-3 max-w-xs md:max-w-md ${message.speaker === 'User' ? 'bg-[#0058FF] text-white rounded-br-none' : 'bg-[#E2E8F0] text-[#0A1A3F] rounded-bl-none'}`}>
                        <p className="text-sm">{message.text}</p>
                    </div>
                    <span className="text-xs text-gray-400 mt-1">{message.speaker} at {message.timestamp}</span>
                </div>
            ))}
